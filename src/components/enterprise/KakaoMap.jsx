...
function KakaoMap() {
  ...
  const { searchQuery, lastUpdated: searchLastUpdated, displayMode } = useSelector(state => state.search);
  const { lastUpdated: filterLastUpdated } = useSelector(state => state.filteredEnterprise);
  const { 
    reviewLocations,
    bookmarkLocations,
    activeMarkerType,
    lastUpdated: reviewBookmarkLastUpdated 
  } = useSelector(state => state.reviewBookmark);
  ...
  const currentDisplayMode = useMemo(() => {
    ...
    const timestamps = {
      ...
      reviewBookmark: reviewBookmarkLastUpdated
    };

    const latestTimestamp = Math.max(...Object.values(timestamps).filter(Boolean));

    if (!latestTimestamp) return 'initial';

    if (latestTimestamp === reviewBookmarkLastUpdated) {
    ...
    return 'initial';
  }, [isFirstRender, searchLastUpdated, filterLastUpdated, reviewBookmarkLastUpdated, activeMarkerType]);
      ...
      async (map, enterprises, displayMarker, clearMarkers, moveMapToLocation) => {
        ...
        const markerPromises = enterprises.map(async (enterprise, index) => {
          try {
            ...
            // 온/오프라인 구분 없이 항상 offlineMarker 사용
            displayMarker(
              position,
  ...
  const displayReviewMarkers = useCallback(
    async (map, locations, displayMarker, clearMarkers) => {
      clearMarkers();
      
      let firstLocationCoords = null;

      const markerPromises = locations.map(async (location, index) => {
        try {
          ...
          displayMarker(
            position,
            `<div style="padding:5px;font-size:12px;">
              ${location.name || location.companyName}<br>
      ...
      if (firstLocationCoords) {
        const moveLatLon = new kakao.maps.LatLng(
          firstLocationCoords.latitude, 
          firstLocationCoords.longitude
        );
        map.setLevel(3);
        map.panTo(moveLatLon);
      }
  ...
  const displayBookmarkMarkers = useCallback(
    async (map, locations, displayMarker, clearMarkers) => {
      clearMarkers();
  ...
  useEffect(() => {
    const initializeMap = () => {
      if (window.kakao && window.kakao.maps) {
        ...
        const displayMarker = (locPosition, message, imageSrc) => {
          if (imageSrc === currentLocationMarker) {
            ...
            myLocationMarker = customOverlay;
              ...
              switch (currentDisplayMode) {
                ...
                case 'review':
                  if (reviewLocations.length > 0) {
                    displayReviewMarkers(map, reviewLocations, displayMarker, clearMarkers);
                  }
                  break;

                  case 'bookmark':
                    if (bookmarkLocations.length > 0) {
                      displayBookmarkMarkers(map, bookmarkLocations, displayMarker, clearMarkers);
                    }
                    break;
  
                  case 'enterprises':
                    if (filteredEnterprises.length > 0 && shouldShowMarkers) {
                      displayListMarkers(map, filteredEnterprises, displayMarker, clearMarkers, moveMapToLocation);
                    } else {
                      clearMarkers();
                      moveMapToLocation(lat, lng);
                    }
      ...
      console.log('Filtered enterprises:', filteredEnterprises);
